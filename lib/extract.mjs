...
function extractRHS(src, name) {
  ...
  for (; i < src.length; i++) {
    ...
    // ASI tolerance (the slugify lesson): a semicolonless source ends the statement at the newline. At depth 0
    // on a newline, accept the accumulated slice IF it already evaluates as a complete expression — otherwise
    // keep scanning (a genuinely multi-line literal is still open because depth>0 or it won't parse yet).
    else if (c === '\n' && depth === 0) {
      const candidate = src.slice(start, i).trim();
      if (candidate) { try { evalLiteral(candidate); break; } catch { /* not complete yet — keep scanning */ } }
    }
  }
