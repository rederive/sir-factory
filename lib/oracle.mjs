...
// HELD-OUT ANTI-OVERFIT GATE (QA gate ② v2) — the held-out set's job is to catch an emission that overfit the
// frozen set; the cheapest wrong emission is the constant stub `() => <most-common-output>`. A held-out with a
// single output class cannot kill it (the resynth risk the launch audit named). This checks the HELD-OUT slice
// specifically (coverageGate above checks the whole oracle's surface). A legitimately-constant function (rare)
// can pass --skip with a documented reason. Returns {ok, verdict, why}.
export function heldoutStrengthGate(heldout) {
  if (!heldout || heldout.length === 0) return { ok: false, verdict: 'FAIL', why: 'no held-out set' };
  const clsOf = (e) => JSON.stringify(e && typeof e === 'object' && e.__throw !== undefined ? { __throw: true } : e);
  const counts = {};
  for (const v of heldout) { const k = clsOf(v.expected); counts[k] = (counts[k] || 0) + 1; }
  const classes = Object.keys(counts).length;
  const minShare = Math.min(...Object.values(counts)) / heldout.length;
  // HARD FAIL only on the vacuous case: a one-sided held-out cannot kill a constant stub. Skew is NOT a
  // build-blocker — for single-value-domain predicates (isNull) 1 minority case is the maximum achievable, and
  // refusing it would be the metric punishing a correct oracle (the lesson from the first scorer run). Skew and
  // low-n are advisory (ok:true, verdict WEAK) so the ratchet blocks the real hole without false positives.
  if (classes < 2) return { ok: false, verdict: 'FAIL', why: `one-sided held-out (all ${heldout.length} = same output class) — cannot kill a constant stub` };
  if (classes === 2 && minShare < 0.15) return { ok: true, verdict: 'WEAK', why: `skewed boolean held-out (minority class ${(minShare * 100).toFixed(0)}%) — acceptable if the true-domain is narrow` };
  return { ok: true, verdict: 'OK', why: `${classes} output classes, min share ${(minShare * 100).toFixed(0)}%` };
}
