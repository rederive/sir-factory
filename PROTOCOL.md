...
1. **install** (Bash):
   `node factory.mjs install <name> <ver> --out <root> --unit <unit> [--export <e>] [--hint "<h>"]`
...
3. **stamp** (Bash): `node factory.mjs stamp <workdir> [--frozen 16 --heldout 13]`
...
4. **re-emit** (Agent tool → `sir-reemitter` ×N, in parallel): give each the SIR (read the SIR_PATH) + the
   ...
   - **Carried data:** if the decomposer declared carried constants (`sir/<unit>.carried.json`), run
     `node factory.mjs extract <workdir>` first — it byte-exact-extracts them to `src/<unit>.data.js` (+ a
   ...
   - **Large SIRs (inline gets unwieldy / risks transcription error — e.g. bcrypt's 28 KB SIR):** run
     `node factory.mjs stage-reemit <workdir>` to write a CLEAN ROOM (`<workdir>/reemit/`: the SIR + a rendered
...
5. **grade** (Bash): `node factory.mjs grade <workdir> --round <r> [--cap 3] [--differential 400]`
...
`node factory.mjs status --out <root>` summarizes progress; the loop is resumable (skip verified/quarantined)
